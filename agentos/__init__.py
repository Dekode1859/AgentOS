...
def _read_version() -> str:
    ...
    The distribution name is spelled out rather than derived from `__name__`.
    They genuinely differ (`agentos-desktop` ships the `agentos` package), and
    an unrelated `agentos` distribution exists on PyPI, so looking the metadata
    up by the import name could report a stranger's version number.
    """
    from importlib.metadata import PackageNotFoundError, version

    try:
        return version("agentos-desktop")
