...
1. ``AGENTOS_OPENCODE_BIN`` — an operator-supplied path, always wins.
...
``agentos install-engine``). A runtime that silently pulls 60 MB the first time
...
def cache_root() -> Path:
    ...
    Shared across every AgentOS app on the machine — the engine is a tool, not
...
def version_warning(version: str | None) -> str | None:
    ...
    if parsed < MIN_VERSION:
        return (
            ...
            f"{'.'.join(map(str, MIN_VERSION))}. Upgrade with: agentos install-engine"
        )
...
def install(version: str = PINNED_VERSION, force: bool = False,
    ...
    with tempfile.TemporaryDirectory(prefix="agentos-engine-") as tmp:
        tmpdir = Path(tmp)
