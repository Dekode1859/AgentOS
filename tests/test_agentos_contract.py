...
import agentos
from agentos import AppConfig, WorkspaceFolder

PACKAGE_DIR = Path(agentos.__file__).resolve().parent


def test_public_api_is_exactly_the_documented_surface():
    assert set(agentos.__all__) == {"run", "AppConfig", "WorkspaceFolder"}
    for name in agentos.__all__:
        assert hasattr(agentos, name)


def test_version_is_pep440_parseable():
    assert re.fullmatch(r"\d+\.\d+\.\d+([abrc.].*)?", agentos.__version__), agentos.__version__


def test_importing_agentos_does_not_require_a_gui_toolkit():
    """`import agentos` must work in CI, packaging, and test contexts.
