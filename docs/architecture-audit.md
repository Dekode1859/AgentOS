# AgentOS — Architecture Audit (Phase 1)
...
The governing constraint comes from the AgentOS Runtime spec:

> **AgentOS Core is a fully generic execution runtime. It must never contain
...
```
EDU/main.py                       → core/agentos/runtime/shell.py        (+ run())
EDU/core/opencode_server.py       → core/agentos/runtime/server.py
EDU/core/paths.py                 → core/agentos/runtime/paths.py        (app_id injected)
EDU/core/vault.py  (primitives)   → core/agentos/storage/__init__.py     (generic, no folders)
EDU/core/vault.py  (FOLDERS/taxo) → apps/learning-os/  (app config: workspace_folders)
EDU/api/bridge.py  (providers)    → core/agentos/providers/__init__.py
EDU/api/bridge.py  (agents read)  → core/agentos/agents/__init__.py
EDU/api/bridge.py  (bridge glue)  → core/agentos/bridge.py               (uses AppConfig)
EDU/ui/*                          → core/agentos/ui/*                    (branding via config)
EDU/opencode.json                 → apps/learning-os/opencode.json       (DOMAIN, unchanged)
EDU/vault/*                       → apps/learning-os/workspace/*         (DOMAIN data)

(tools)                           → core/agentos/tools/   README: delegated to OpenCode
(events)                          → core/agentos/events/  README: OpenCode SSE + UI bus
```

### The Core↔App contract

The single seam between Core and an app is **`agentos.AppConfig`**. An app
constructs one object and calls `agentos.run(config)`. Core reads *only* this
...
| Swap Core → apps unmodified | Apps depend only on the public API `agentos.run` / `AppConfig` / `WorkspaceFolder`. Any Core implementing that contract is valid. |
| Storage has no folder semantics | `core/agentos/storage` exposes read/write/list/delete over a root. Folder names come from `AppConfig`, supplied by the app. |
