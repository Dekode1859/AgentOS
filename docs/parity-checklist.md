...
| 2 | OpenCode server boots, isolated home | `core/opencode_server.py` | `agentos.runtime.OpenCodeServer` (same logic, `.opencode-home/` per app) | ✅ same code path |
...
| 5 | Model picker lists providers; "Free" badge for OpenCode Zen only | `bridge.get_providers` | `agentos.providers.list_providers` (same logic) | ✅ same code path |
...
| 15 | Provider key save/remove restarts server | `bridge` | `agentos.providers` + `Bridge` (same logic) | ✅ same code path |
| 16 | Set default model rewrites opencode.json | `bridge.set_default_model` | `agentos.providers.set_default_model` (same logic) | ✅ same code path |
...
- `import agentos` succeeds without a GUI (lazy `webview` import). ✅
